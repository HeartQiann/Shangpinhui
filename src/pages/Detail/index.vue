...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果服务器的数据没有回来。skuinfo对象是空的
      return this.skuInfo.skuImageList || [{}];
    },
  },
  methods: {
    //售卖属性值的排他操作
    changeActive(saleAttr, spuSaleAttrValueList) {
      //设置所有的ischecked都为0
    ...
    async addShopCart() {
      try {
        //发请求 派发action 通知服务器加入购物车的商品是什么
        let result = await this.$store.dispatch("getShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //成功：进行路由跳转+传参 但是第一种方法导致地址栏很乱
