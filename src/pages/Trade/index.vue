<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        @click="changeAddress(index, useraddress)"
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("userTrade");
  },
  computed: {
    ...mapState({
      useraddress: (state) => state.trade.useraddress,
      usertrade: (state) => state.trade.usertrade,
    }),
    ...
    allPrice() {
      let sum = 0;
      for (let i = 0; i < this.detailArrayList.length; i++) {
        sum +=
          this.detailArrayList[i].orderPrice * this.detailArrayList[i].skuNum;
      }
      return sum;
    },
  },
  methods: {
    changeAddress(index, useraddress) {
      for (let i = 0; i < useraddress.length; i++) {
        useraddress[i].isDefault = 0;
      }
      useraddress[index].isDefault = 1;
    },
    //提交订单
    async submitOrder() {
